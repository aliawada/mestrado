...
NoRB* remover_rb(NoRB* raiz, int valor) {
    if (!raiz) return NULL;

    if (valor < raiz->valor)
        raiz->esq = remover_rb(raiz->esq, valor);
    else if (valor > raiz->valor)
        raiz->dir = remover_rb(raiz->dir, valor);
    else {
        if (!raiz->esq || !raiz->dir) {
            NoRB* temp = raiz->esq ? raiz->esq : raiz->dir;
            free(raiz);
            return temp;
        } else {
            NoRB* sucessor = raiz->dir;
            while (sucessor->esq) sucessor = sucessor->esq;
            raiz->valor = sucessor->valor;
            raiz->dir = remover_rb(raiz->dir, sucessor->valor);
        }
    }

    return balancear_rb(raiz); // usa balanceamento básico
}
