...
void initialize_nodes(int width, int height, int grid[][10], char visual[][10], Point start, Point end) {
    ...
    // Marcar início e fim visualmente no mapa
    ...
    nodes[start.y][start.x].h = heuristic(start, end); // estimativa de distância até o fim
    ...
    nodes[start.y][start.x].in_open = 1; // nó está na lista aberta (ainda será avaliado)
}

/// Essa função calcula a "distância" entre dois pontos
/// Aqui usamos a distância de Manhattan (sem diagonais)
