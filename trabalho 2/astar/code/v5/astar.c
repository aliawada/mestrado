...
Node nodes[10][10];
int alternative_g_values[100];
int alternative_g_count = 0;
...
int a_star(int grid[][10], int width, int height, Point start, Point end, char visual[][10]) {
    ...
    nodes[start.y][start.x].f = nodes[start.y][start.x].g + nodes[start.y][start.x].h;
    ...
    while (1) {
        ...
        if (cx == end.x && cy == end.y) {
            // Registrar como caminho possível
            alternative_g_values[alternative_g_count++] = nodes[cy][cx].g;

            // Só reconstruir e marcar o visual no primeiro caminho encontrado
            if (!resolved) {
                reconstruct_path(nodes, end, visual, start);
                resolved = 1;
            }
            continue; // Continue buscando caminhos alternativos
        }
		int i;
        for ( i = 0; i < 4; i++) {
            int nx = cx + dx[i];
